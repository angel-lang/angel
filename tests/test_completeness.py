...
from compiler import type_checking, nodes, estimation, repl_evaluation, analysis, translators
...
class TestCompleteness(unittest.TestCase):
    ...
    def test_type_inference(self):
        self.assertEqual(
            self.type_checker.supported_nodes_by_type_inference, get_all_subclasses(nodes.Expression) - set(
                subclass.__name__ for subclass in (nodes.ConstantDeclaration,)
            )
        )

    def test_type_unification(self):
        expected = set()
        not_supported = set(subclass.__name__ for subclass in (nodes.Name,))
    ...
    def test_translation(self):
        self.assertEqual(self.translator.supported_nodes, get_all_subclasses(nodes.Node))
